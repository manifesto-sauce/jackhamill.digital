...
export const postsQuery = groq`*[_type == 'posts']{title, banner, 'slug': slug.current, subtitle, date, 'category': category->slug.current}`
export const postQuery = groq`*[_type == 'posts' && slug.current == $slug][0]{..., title, banner, 'slug': slug.current, subtitle, date, 'category': category->slug.current }`
export const eventsQuery = groq`*[_type == 'events']{'slug': slug.current, title, subtitle,banner, 'category': category->slug.current}`
export const eventQuery = groq`*[_type == 'events' && slug.current == $slug][0]{
  title, subtitle, banner, content, date, 'slug': slug.current, 'category': category->slug.current
}`
